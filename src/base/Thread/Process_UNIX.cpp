...
int ProcessHandleImpl::wait() const
{
	...
	if (rc != _pid)
        throw SystemException("Cannot wait for process", itos(_pid));
...
ProcessHandleImpl* ProcessImpl::launchByForkExecImpl(const std::string& command, const ArgsImpl& args,
                                                     const std::string& initialDirectory,
                                                     Pipe* inPipe, Pipe* outPipe, Pipe* errPipe,
                                                     const EnvImpl& env)
{
