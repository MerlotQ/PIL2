...
///
/// FIXME:
///     1. currently, the RDataStream do not support little-endian and big-endian
///     2. support SPtr
///
class RDataStream
{
public:
    ...
    void clear_noDel(void) {
        if( m_fromRawData ) {
            pi_dbg_error("Datastream from raw data! Please do not modify it\n");
            exit(1);
        }
    ...
    RDataStream& fromRawData(uint8_t *d, uint32_t l, int doCopy=1) {
        if( doCopy ) {
            ...
            m_fromRawData = 0;                      // FIXME: this means using user alloc data
            m_arrData = d;
            m_size = l;
            m_sizeReserved = l;
